...
with open('database/flute.csv') as csv_file:
    ...
    for col in reader:
        if reader.line_num > 1:
            session.add(Flute(order_id=col[1], customer_id=col[2], flute_type=col[3], hand=col[4], key=col[5],
                              octave=col[6], scale_name=col[7], tuning_ref=col[8], flute_wood=col[9],
                              block_wood=col[10]))
