...
file_path = r'C:\Users\carl.young\Documents\sapele\New Sapele\csv stuff\my_orders.csv'
...
def view_orders():
    ...
    with open(file_path, 'r') as csv_file:
        ...
        for row in csv_reader:
            print(f"{num}: {row.get('Name')}, {row.get('Flute Type')}, {row.get('Key')}, {row.get('Octave')}, "
                  f"{row.get('REF')}, {row.get('Scale')}, {row.get('Flute Wood')}, {row.get('Block Wood')}"
                  f", {row.get('Qty')}, {row.get('Price')}, {row.get('Total Price')}, {row.get('Order Date')}, "
                  f"{row.get('Ship Date')}")
            num += 1
...
def view_customer_orders():
    ...
    print()
    print(orders[orders['Name'].str.contains(name)])
    input('\nPress Enter to continue...')
